<?php
	...
	if(!empty($_GET['cod']) and !empty($_GET['periodo'])){
		...
		$oPeriodo=new Consultar_Periodo($id_periodo, $conexion);
		$oAlumno=new Consultar_Alumno($id_alumno, $conexion);	
		$nombre_alumno=$oAlumno->consultar('nombre');
		
		$oGrado= new Consultar_Grado($oAlumno->consultar('grado'), $conexion);
		$oSalon= new Consultar_Salon($oAlumno->consultar('salon'), $conexion);
		
		$nombre_salon=$oSalon->consultar('nombre');
		$nombre_grado=$oGrado->consultar('nombre');
		
		$oEmpresa= new Consultar_Empresa('1', $conexion);
...
<body>
    ...
    <div align="center" id="imprimeme">
        <table width="90%" style="font-family:Arial, Helvetica, sans-serif">
          ...
          <?php 
		  	$pa=mysqli_query($conexion, "SELECT * FROM calificar WHERE alumno='$id_alumno' and periodo='$id_periodo'");				
